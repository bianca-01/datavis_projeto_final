// ─────────────────────────────────────────────
//  home.js — Decifrando o Oscar
//  1. Contagem animada das estatísticas
//  2. Reveal escalonado das barras
//  3. Menu mobile
//  4. Marca link ativo na nav
// ─────────────────────────────────────────────
...
const sortear = document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("filmes.json")
    ...
    .then((data) => {
      // Como o JSON agrupa linhas por indicação, criamos uma lista de filmes únicos
      const filmMap = {};
      data.forEach((row) => {
        const key = row.tmdb_id || row.imdb_id || row.film;
        if (!filmMap[key]) {
          // Simplifica a estrutura para o grid
          const nominations = data.filter((r) => (r.tmdb_id || r.imdb_id || r.film) === key);
          const totalWins = nominations.filter((r) => r.winner === "True" || r.winner === true).length;

          filmMap[key] = {
            id: encodeURIComponent(row.tmdb_id || row.imdb_id || row.film || ""),
            titulo: row.titulo || row.film || "Sem título",
            poster: row.poster_url || "https://image.tmdb.org/t/p/w342/" + row.poster_path, // Fallback caso use path
            vitorias: totalWins
          };
        }
      });
      ...
      gridContainer.innerHTML = selectedFilms.map((movie) => `
    ...
    .catch((error) => console.error("Não foi possível carregar a prévia dos filmes:", error));
});