// ─────────────────────────────────────────────────────────────
//  filme.js — Decifrando o Oscar
//  Página de detalhes — lê ?id= da URL, carrega filmes.json
//  O ?id= pode ser: tmdb_id (número), imdb_id (ttXXXXX) ou
//  slug (título encodado) — aceita qualquer um.
// ─────────────────────────────────────────────────────────────
...
const LANGS = {
  ...
  zh:'Mandarim',ar:'Árabe',     ru:'Russo',   sv:'Sueco',
  da:'Dinamarquês', nl:'Holandês', no:'Norueguês', hi:'Hindi',
}
const COUNTRIES = {
  'United States of America':'EUA','United Kingdom':'Reino Unido',
  'France':'França','Germany':'Alemanha','Italy':'Itália',
  'Japan':'Japão','South Korea':'Coreia do Sul','Spain':'Espanha',
  'Australia':'Austrália','Canada':'Canadá','Ireland':'Irlanda',
  'Mexico':'México','Brazil':'Brasil','India':'Índia',
  'New Zealand':'Nova Zelândia','Austria':'Áustria','China':'China',
  'Argentina':'Argentina','Poland':'Polônia','Sweden':'Suécia',
  'Denmark':'Dinamarca','Netherlands':'Holanda',
}
const STREAMING_ICONS = {
  Netflix:'N','Prime Video':'▶','Disney+':'✦',
  'Apple TV+':'','Apple TV':'','Max':'M',
  Peacock:'P',Mubi:'M',Tubi:'T','Paramount+':'★',Hulu:'H',
}
const DEPT_PT = {
  Directing:'Direção', Writing:'Roteiro', Production:'Produção',
  Camera:'Fotografia', Art:'Arte', 'Costume & Make-Up':'Figurino',
  Sound:'Som', Editing:'Montagem', 'Visual Effects':'Efeitos Visuais',
  'Crew':'Equipe', 'Lighting':'Iluminação',
}
const DEPT_ORDER = [
  'Directing','Writing','Production','Camera','Art',
  'Costume & Make-Up','Sound','Editing','Visual Effects',
]
...
function country(arr) {
  return (arr || []).slice(0, 2).map(c => COUNTRIES[c] || c).join(' / ') || '—'
}
...
function ordinal(n) { return n + 'ª ed.' }
...
function render(film, allFilms) {
  ...
  const writers = film.equipe_tecnica.filter(e => e.departamento === 'Writing').slice(0, 3)
  ...
  // Prev / Next
  const idx  = allFilms.findIndex(f => f.tmdb_id === film.tmdb_id || f.film === film.film)
        ...
        <div class="detail-hero__info">
          ...
          <div class="detail-hero__meta">
            ${[film.year_film, rt, countries]
              .filter(Boolean)
              .map((v, i) => i === 0 ? v : `<span class="detail-hero__meta-sep">·</span>${v}`)
              .join(' ')}
          </div>
    ...
    <div class="detail-body">
      ...
      <section class="detail-section">
        ...
        <div class="detail-tags">
          ...
          ${film.palavras_chave.slice(0, 12).map(k => `<span class="tag-keyword">${k}</span>`).join('')}
        </div>
      </section>` : ''}
  ...
  el('detail-footer').innerHTML = `
    ...
    <div class="detail-footer__nav">
      ${prev ? `<a href="filme.html?id=${navId(prev)}" class="detail-nav-btn">
      ...
      ${next ? `<a href="filme.html?id=${navId(next)}" class="detail-nav-btn">
        próximo filme
        <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
...
if (!idStr || idStr === 'null' || idStr === 'undefined') {
  showError('Parâmetro ausente', 'Nenhum filme foi selecionado. Escolha um na lista.')
} else {
  fetch('filmes.json')
    .then(r => {
      if (!r.ok) throw new Error('Arquivo filmes.json não encontrado (HTTP ' + r.status + ').')
      return r.json()
    })
    .then(data => {
      if (!Array.isArray(data)) {
        throw new Error('filmes.json precisa ser um array de registros.')
      }
      ...
      if (!rows || rows.length === 0) {
        showError(
          ...
          'ID "' + idStr + '" não existe no dataset. Verifique se o arquivo filmes.json está correto.'
        )
        return
      }
      ...
      const allFilms = Object.values(filmMap)
        .map(buildFilm)
        .sort((a, b) => b.total_wins - a.total_wins || b.total_nominations - a.total_nominations)
